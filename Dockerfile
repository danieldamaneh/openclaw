...
RUN --mount=type=cache,id=cache-key:pnpm-store,target=/root/.local/share/pnpm/store,sharing=locked \
...
RUN --mount=type=cache,id=cache-key:apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=cache-key:apt-lists,target=/var/lib/apt,sharing=locked \
    apt-get update && \
